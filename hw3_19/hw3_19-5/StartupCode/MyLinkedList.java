public class MyLinkedList {
	...
	private MyLinkedList reverse(ListNode node) {
		//this works
		if(node == null) {
			return new MyLinkedList(); //returns an empty linked list
		}else {
			MyLinkedList subReversed = reverse(node.next); //API call
			subReversed.addLast(node.data); //another helper method addLast
			return subReversed;
		}
	}
	...
	private ListNode reverse(ListNode first, ListNode second) {
		//this works
		if(second == null) { //if the list has one node in it, we return that one node
			return first;
		}
		ListNode newHead = reverse(second, second.next);
		second.next = first; //reverses the link
		first.next = null;
		return newHead;
	}
